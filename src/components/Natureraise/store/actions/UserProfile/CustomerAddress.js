...
export const addCustomerAddress=(address_id,contact_name,mobile_number,address_line1,address_line2,city,state,pincode,landmark,type)=>dispatch=>
{
    ...
    fetch(Config.BaseUrl + "graphql", {
      ...
      .then((responseText) => {
        ...
        if (responseText.data.addCustomerAddress["message"] === "SUCCESS") {
            dispatch({ type: "SUCCESS_MESSAGE", success_title: "ADD_SUCCESS_MESSAGE"},);
        } else {
             dispatch({ type: "SUCCESS_MESSAGE", success_title: "ADD_ERROR_MESSAGE" },);
            dispatch({ type: "ERROR_MESSAGE", error_title: responseText.data.addCustomerAddress["message"] });
        }
      })
      .catch((error) => {
        dispatch({ type: "SUCCESS_MESSAGE", success_title: "ADD_CATCH_MESSAGE" },);
  ...
  export const getCustomerAddressList=()=>dispatch=>
{
    ...
    Config.client
      ...
      .then((result) => {
        dispatch({ type: "GET_CUSTOMER_ADDRESS_LIST", address_data_list: result.data.getCustomerAddressList },);
      })
      .catch((error) => {
    ...
    fetch(Config.BaseUrl + "graphql", {
      ...
      .then((responseText) => {
        ...
        if (responseText.data.deleteCustomerAddress["message"] === "SUCCESS") {
            dispatch({ type: "SUCCESS_MESSAGE", success_title: "DELETE_SUCCESS_MESSAGE" },);
        } else {
            dispatch({ type: "ERROR_MESSAGE", success_title: responseText.data.deleteCustomerAddress["message"] },);
        }
      })
      .catch((error) => {
        dispatch({ type: "SUCCESS_MESSAGE", success_title: "catch error" },);
